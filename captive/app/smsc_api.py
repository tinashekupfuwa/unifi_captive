# -*- coding: utf-8 -*-
# BILLING.SMSTRAF.RU API (billing.smstraf.ru) версия 1.9 (01.02.2017)
...
SMTP_FROM = "api@billing.smstraf.ru"		# e-mail адрес отправителя
SMTP_SERVER = "send.billing.smstraf.ru"	# адрес smtp сервера
...
# Класс для взаимодействия с сервером billing.smstraf.ru
...
class SMSC(object):
	...
	def send_sms_mail(self, phones, message, translit=0, time="", id=0, format=0, sender=""):
		...
		server.sendmail(SMTP_FROM, "send@send.billing.smstraf.ru", "Content-Type: text/plain; charset=" + SMSC_CHARSET + "\n\n" + \
	...
	def _smsc_send_cmd(self, cmd, arg=""):
		url = ifs(SMSC_HTTPS, "https", "http") + "://billing.smstraf.ru/sys/" + cmd + ".php"
		_url = url
		...
		while ret == "" and i <= 5:
			if i > 0:
				url = _url.replace("billing.smstraf.ru/", "www" + str(i) + ".billing.smstraf.ru/")
			else:
				i += 1
...
# smsc.send_sms("79999999999", "http://billing.smstraf.ru\nBILLING.SMSTRAF.RU", query="maxsms=3")
